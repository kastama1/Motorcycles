<footer class="p-4 bg-black shadow md:flex md:items-center md:justify-between md:p-6">
    <span class="text-sm text-white sm:text-center">© 2022 <x-nav-link href="{{ route('index') }}"  :active="request()->routeIs('index')">Classic-motorcycles</x-nav-link> - 
            @if (Auth::check())
                <a href="{{ route('logout') }}"
            ...
            @else
                <x-nav-link href="{{ route('login') }}"  :active="request()->routeIs('login')">Přihlásit se pro správu</x-nav-link>
            @endif
    </span>
